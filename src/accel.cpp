...
void Accel::addMesh(Mesh *mesh) {
    ...
    // if the mesh is the first, initialize bbox
    if (m_meshes.size() == 1) {
        m_bbox = mesh->getBoundingBox();
    } else {
        // otherwise expand bbox
...
void Accel::build() {
    ...
    // for all meshes & indices
    std::vector<std::pair<const Mesh*, uint32_t>> triangle_list;

    for (const auto& mesh : m_meshes) {
        uint32_t triCount = mesh->getTriangleCount();
        for (uint32_t i = 0; i < triCount; ++i) {
            triangle_list.emplace_back(mesh, i); // save mesh pointer and its triangle indices
        }
    }
    ...
    octree = build(m_bbox, triangle_list, 0);

    cout << "done. (took " << timer.elapsedString() << ")" << endl;
}

Node* Accel::build(BoundingBox3f bbox, std::vector<std::pair<const Mesh*, uint32_t>> list, uint32_t maxDepth) {
    if (list.empty())
        return nullptr;
    ...
    std::vector<std::pair<const Mesh*, uint32_t>> newList[8];
    for (auto& pair : list) {
        const Mesh* mesh = pair.first;
        uint32_t fIdx = pair.second;
        for (size_t i = 0; i < 8; ++i) {
            if (mesh->getBoundingBox(fIdx).overlaps(childBbox[i]))
                newList[i].emplace_back(mesh, fIdx);
        }
    }
...
bool Accel::rayIntersect(const Ray3f &ray_, Intersection &its, bool shadowRay) const {
    ...
    while (!queue.empty()) {
        ...
        // if the nearest intersection in this node is farther than the closest found so far, skip
        ...
        if (!node->isChild) {
            ...
            for (size_t i = 0; i < 8; ++i) {
                if (node->child[i] == nullptr)
                    continue;
                float nearT, farT;
                if (node->child[i]->bbox.rayIntersect(ray, nearT, farT)) {
                    // if the nearT is farther than the closest found so far, skip
                    if (nearT > ray.maxt)
                        continue;
            ...
            // traverse (mesh, triangle) pairs
            for (auto &[mesh, fIdx] : node->triangle_list) {
                float u, v, t;
                if (mesh->rayIntersect(fIdx, ray, u, v, t)) {
    ...
    if (foundIntersection && hitMesh) {
        // Vector3f bary;
        // bary << 1 - its.uv.sum(), its.uv;

        // const Mesh *mesh   = hitMesh;
