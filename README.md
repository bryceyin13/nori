# Nori
Assignments for Nori.

### Assignment 1

### Assignment 2

#### Part 1: Octree construction

#### Part 2: Ray traversal

#### Part 3: Improved ray traversal

#### Part 4: Efficiency

#### Hacker Points

### Assignment 3

#### Part 1: Monte Carlo Sampling

#### Part 2: Two simple rendering algorithms

##### Part 2.1: Point lights

##### Part 2.2: Ambient occlusion

### Before Assignment 4: Support for Multi-Mesh Scenes

### Assignment 4

#### Part 1: Area lights

#### Part 2: Distribution Ray Tracing

#### Part 3: Dielectrics

#### Part 4: Whitted-style ray tracing

#### Artist Points: Interesting scene

#### Hacker Points: Specialized Light Source Sampling

### Assignment 5

#### Part 1: Microfacet BRDF

#### Part 2: Brute force path tracer

#### Part 3: Path tracer with next event estimation

#### Part 4: Path tracer with Multiple Importance Sampling

#### Artist Points: Interesting scene

#### Hacker Points: Refraction through rough dielectrics